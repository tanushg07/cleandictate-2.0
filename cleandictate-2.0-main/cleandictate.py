...
class CustomDictionary:
    def __init__(self, path=None):
        self.path = path or os.path.join(os.path.dirname(__file__), "custom_dictionary.json")
        self.vocabulary, self.corrections = [], {}
        self.load()
    def load(self):
        try:
            if os.path.exists(self.path):
                with open(self.path, "r", encoding="utf-8") as f: data = json.load(f)
                self.vocabulary = [str(x).strip() for x in data.get("vocabulary", []) if str(x).strip()]
                self.corrections = {str(k).strip():str(v).strip() for k,v in data.get("corrections", {}).items() if str(k).strip() and str(v).strip()}
            else: self.save()
        except Exception as e:
            print(f"[Dictionary] Could not load dictionary: {e}")
            self.vocabulary, self.corrections = [], {}
    def save(self):
        with open(self.path, "w", encoding="utf-8") as f: json.dump({"vocabulary":self.vocabulary,"corrections":self.corrections}, f, ensure_ascii=False, indent=2)
    def snapshot(self): return {"vocabulary":self.vocabulary,"corrections":self.corrections}
    def add_word(self, word):
        word=str(word).strip()
        if word and word.lower() not in {x.lower() for x in self.vocabulary}: self.vocabulary.append(word); self.save(); return True
        return False
    def remove_word(self, word):
        before=len(self.vocabulary); self.vocabulary=[x for x in self.vocabulary if x != word]
        if len(self.vocabulary)!=before: self.save(); return True
        return False
    def set_correction(self, heard_as, replace_with):
        heard_as,replace_with=str(heard_as).strip(),str(replace_with).strip()
        if not heard_as or not replace_with: return False
        self.corrections[heard_as]=replace_with; self.save(); return True
    def remove_correction(self, heard_as):
        if heard_as in self.corrections: del self.corrections[heard_as]; self.save(); return True
        return False
    def prompt(self, base_prompt):
        if not self.vocabulary: return base_prompt
        return f"{base_prompt} Important names, acronyms, brands, and technical terms that may appear are: {', '.join(self.vocabulary)}. Preserve their spelling when recognized."
    def apply_corrections(self, text):
        corrected_text, _ = self.apply_corrections_with_tracking(text)
        return corrected_text

    def apply_corrections_with_tracking(self, text):
        """Apply explicit Heard as → Replace with rules and record actual changes.

        Vocabulary entries are only recognition hints and are deliberately not
        included here. Only text genuinely changed by a correction rule is tracked.
        """
        changes = []
        for heard_as, replacement in sorted(self.corrections.items(), key=lambda kv: len(kv[0]), reverse=True):
            pattern = r'(?<!\w)' + re.escape(heard_as) + r'(?!\w)'
            count = 0
...
[Your Name]"""
    ...
    def __init__(self):
        """Initialize both models on CPU with float32 precision"""
        ...
        print("[Style] Model A loaded (CPU float32)")
        ...
        print("[Style] Model B loaded (CPU float32)")
    ...
    def _warmup(self):
        ...
        inputs_a = self.grammar_tokenizer(dummy_grammar, return_tensors="pt").to("cpu")
        with torch.no_grad():
        ...
        inputs_b = self.style_tokenizer(dummy_style, return_tensors="pt").to("cpu")
        with torch.no_grad():
    ...
    def _correct_grammar(self, text: str) -> str:
        ...
        inputs = self.grammar_tokenizer(input_text, return_tensors="pt", max_length=256, truncation=True).to("cpu")
...
BODY: [body here]"""
        ...
        inputs = self.style_tokenizer(prompt, return_tensors="pt", max_length=512, truncation=True).to("cpu")
...
class SpeechEngine:
    ...
    def __init__(self, gui_callback=None):
        ...
        # Check CUDA (optional - falls back to CPU if unavailable)
        if torch.cuda.is_available():
            print(f"[GPU] CUDA Device: {torch.cuda.get_device_name(0)}")
            whisper_device = "cuda"
            whisper_compute_type = "float16"
        else:
            print("[CPU] No CUDA GPU detected - running on CPU (this will be slower)")
            whisper_device = "cpu"
        ...
        print(f"[Dictionary] Loaded {len(self.custom_dictionary.vocabulary)} vocabulary entries and {len(self.custom_dictionary.corrections)} correction rules.")
    ...
    # ---------- Custom dictionary ----------
    def get_custom_dictionary(self): return self.custom_dictionary.snapshot()
    def add_vocabulary_word(self, word): return self.custom_dictionary.add_word(word)
    def remove_vocabulary_word(self, word): return self.custom_dictionary.remove_word(word)
    def set_dictionary_correction(self, heard_as, replace_with): return self.custom_dictionary.set_correction(heard_as, replace_with)
    def remove_dictionary_correction(self, heard_as): return self.custom_dictionary.remove_correction(heard_as)
        ...
        while self.is_running:
            try:
                ...
                segments, info = self.whisper_model.transcribe(
                    ...
                    beam_size=1,
                    language="en",
                    initial_prompt=self.custom_dictionary.prompt(self.initial_prompt),
                    vad_filter=False,
                ...
                whisper_text = " ".join(text_parts).strip()
                raw_text, corrections_applied = self.custom_dictionary.apply_corrections_with_tracking(whisper_text)
        ...
        try:
            ...
            if self.gui_callback:
                self.gui_callback({
                    ...
                    'raw_text': raw_text,
                    'corrections_applied': corrections_applied,
                    'asr_latency_ms': display_asr,
                    'clean_latency_ms': display_clean,
                    'style_latency_ms': display_style,
                    'mode': self.current_mode,
                    'tone': self.current_tone,
                })
